...
import { ExternalLink, Play, FileText, Globe, Code, Database, Terminal, Palette, Zap } from "lucide-react";
...
export function ActivityExpandableCards({ activities }: { activities: Activity[] }) {
  const [active, setActive] = useState<(typeof activities)[number] | boolean | null>(null);
  ...
  // Helper function to get resource icon based on URL
  ...
  return (
    <div className="max-w-4xl mx-auto w-full space-y-4 px-2 sm:px-0">
      {activities.map((activity, index) => (
        <div key={`card-${activity.name}-${id}`} className="relative">
          <motion.div
            onClick={() => setActive(active === activity ? null : activity)}
          ...
          >
            ...
            <div className="flex gap-3 sm:gap-4 flex-col md:flex-row w-full">
              <div className="flex items-center flex-1 min-w-0">
                <div className="text-gray-400 mr-2 sm:mr-3 flex-shrink-0">
                  {activity.icon}
                </div>
              ...
              <button className="px-3 py-1 text-xs sm:text-sm rounded-md font-medium bg-gray-700 hover:bg-gray-600 text-gray-200 mt-3 sm:mt-4 md:mt-0 transition-colors duration-300 whitespace-nowrap">
                {active === activity ? 'Hide Resources' : 'View Resources'}
              </button>
            </div>
          </motion.div>
                ...
                <div className="bg-gray-800/30 border border-gray-700 rounded-xl p-4 sm:p-6 mt-2">
                  <div className="mb-4">
                    ...
                    <p className="text-gray-300 text-sm sm:text-base leading-relaxed">
                      Comprehensive resources for {activity.name.toLowerCase()}
                    </p>
                  </div>
                  ...
                  <div className="max-h-96 overflow-y-auto [scrollbar-width:thin] [scrollbar-color:gray-600_gray-800]">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pr-2">
                      {activity.resources.map((resource: ActivityResource, resourceIndex: number) => {
                        return (
                          <a
                            key={resourceIndex}
                            href={resource.url}
